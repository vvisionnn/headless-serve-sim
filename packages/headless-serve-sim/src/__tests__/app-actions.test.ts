...
import { parseAppActionsArgs } from "../app-actions";
...
describe("parseAppActionsArgs", () => {
  ...
  test("push with a reverse-dns bundleId is accepted; an invalid one is rejected", () => {
    ...
    expect(
      parseAppActionsArgs(["push", "com apple", "--payload", '{"aps":{}}']),
    ).toEqual({ error: expect.stringContaining("Invalid bundle id") });
  });
  ...
  test("push --payload with invalid JSON is rejected and does not reach simctl", () => {
    expect(
      parseAppActionsArgs(["push", "com.apple.mobilesafari", "--payload", "notjson"]),
    ).toEqual({ error: expect.stringContaining("Invalid JSON") });
  });
  ...
  test("push --payload with a non-object (array) JSON is rejected", () => {
    expect(
      parseAppActionsArgs(["push", "com.apple.mobilesafari", "--payload", "[1,2,3]"]),
    ).toEqual({ error: expect.stringContaining("object") });
  });
  ...
  test("push with BOTH --payload and --file is rejected (exactly one required)", () => {
    expect(
      parseAppActionsArgs([
        "push", "com.apple.mobilesafari",
        "--payload", '{"aps":{}}', "--file", "/tmp/p.apns",
      ]),
  ...
  test("-d/--device is captured (short and long form)", () => {
    expect(
      parseAppActionsArgs(["open-url", "https://x.test", "-d", "iPhone 15"]),
    ).toMatchObject({ device: "iPhone 15" });
    expect(
      parseAppActionsArgs(["keychain-reset", "--device", "UDID-1"]),
    ).toMatchObject({ device: "UDID-1" });
  });

  test("--device=UDID form is captured", () => {
    expect(
      parseAppActionsArgs(["keychain-reset", "--device", "UDID-2"]),
    ).toMatchObject({ device: "UDID-2" });
  });
