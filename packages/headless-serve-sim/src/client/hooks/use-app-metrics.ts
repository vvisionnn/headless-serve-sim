...
export function parseAppMetrics(raw: unknown): AppMetrics | null {
  ...
  const state =
    value.state === "live" || value.state === "no-foreground-app" || value.state === "unavailable"
      ? value.state
      : null;
  ...
  const bundleId =
    value.bundleId === null || typeof value.bundleId === "string" ? value.bundleId : undefined;
  const pid =
    value.pid === null ||
  ...
  if (
    (threadCount !== null && !Number.isInteger(threadCount)) ||
    (runningThreadCount !== null && !Number.isInteger(runningThreadCount))
  )
    return null;
  if (threadCount !== null && runningThreadCount !== null && runningThreadCount > threadCount)
    return null;
  ...
  if (
    memoryFootprintBytes !== null &&
    peakMemoryFootprintBytes !== null &&
    peakMemoryFootprintBytes < memoryFootprintBytes
  )
    return null;
...
export function useAppMetrics(endpoint: string | undefined, enabled: boolean): AppMetricsStream {
