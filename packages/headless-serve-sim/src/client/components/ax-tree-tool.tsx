...
export function AxTreeTool({
  ...
  return (
    <div className={SECTION}>
      <div className="flex items-center justify-between gap-2.5 px-5 min-h-[56px] select-none">
        <span className={SECTION_TITLE}>AX Tree</span>
        <button
          type="button"
          onClick={onToggleOverlay}
          aria-pressed={overlayEnabled}
          className="rounded-card border border-divider bg-transparent text-fg-2 cursor-pointer text-value px-3 py-1 transition-[background-color,color] duration-300 ease-[cubic-bezier(0.4,0,0.6,1)] hover:bg-hover focus-visible:outline-none focus-visible:[box-shadow:0_0_0_2px_var(--color-accent-solid)]"
        >
          {overlayEnabled ? "Overlay on" : "Enable overlay"}
        </button>
      </div>
      {!overlayEnabled ? null : axeUnavailable ? (
        <div className="px-5 pb-4 pt-1 flex flex-col gap-3">
          <div className={EMPTY_BLOCK}>AX unavailable on this simulator.</div>
        </div>
      ) : elements.length === 0 ? (
        <div className="px-5 pb-4 pt-1 flex flex-col gap-3">
          <div className={EMPTY_BLOCK}>{error ?? "Waiting for accessibility data…"}</div>
        </div>
      ) : (
        <div className="px-5 pb-4 pt-1 flex flex-col gap-3">
          <div
...
const AxTreeItem = memo(
  function AxTreeItem({
    ...
    const itemTitle = [axNode.label, axNode.role || axNode.type || "element", size]
      .filter(Boolean)
      .join(" · ");
    ...
    return (
      ...
      >
        <span className="flex flex-col gap-0.5 flex-1 min-w-0">
          <span className="overflow-hidden text-ellipsis whitespace-nowrap text-fg text-value font-medium">
            {element.label || element.role || "Unlabeled"}
          </span>
          <span className="overflow-hidden text-ellipsis whitespace-nowrap text-fg-3 font-mono text-micro">
            {element.role || element.type || "element"}
          </span>
        </span>
        <code className="shrink-0 rounded-chip bg-surface-2 border border-divider text-fg-2 font-mono text-micro px-2 py-[3px]">
          {size}
        </code>
      </div>
    );
