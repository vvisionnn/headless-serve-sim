...
const EMPTY_BLOCK =
  "bg-surface-3 border border-divider rounded-card px-3 py-2.5 text-fg-3 text-[12px] text-center";
    ...
    return (
      ...
      >
        <span className="flex flex-col gap-0.5 flex-1 min-w-0">
          <span className="overflow-hidden text-ellipsis whitespace-nowrap text-fg text-[13px] font-medium tracking-[-0.01em]">
            {element.label || element.role || "Unlabeled"}
          </span>
          <span className="overflow-hidden text-ellipsis whitespace-nowrap text-fg-3 font-mono text-[10px]">
            {element.role || element.type || "element"}
          </span>
        </span>
        <code className="shrink-0 rounded-pill bg-surface-2 border border-divider text-fg-2 font-mono text-[10px] px-2 py-[3px]">
          {size}
        </code>
      </div>
    );
