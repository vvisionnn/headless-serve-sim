...
export function WebKitTargetPicker({
  ...
  useLayoutEffect(() => {
    ...
    const place = () => {
      ...
      if (r) setPos({ top: Math.round(r.bottom + 6), left: Math.round(r.left), width: Math.round(r.width) });
    };
    place();
    ...
    <div ref={containerRef} className="relative w-full min-w-0">
      ...
      {open && pos &&
        ...
        <ul
          ref={menuRef}
          role="listbox"
          style={{ position: "fixed", top: pos.top, left: pos.left, width: pos.width, minWidth: 220, zIndex: 1000 }}
            ...
            <li key={group.key} className="list-none m-0 p-0">
              ...
              <ul role="group" className="list-none m-0 p-0 pl-[26px]">
                {group.targets.map((target) => {
                  ...
                  const baseCls = "flex flex-col rounded-sm px-2.5 py-1.5 text-[12px] leading-[1.35] tracking-[-0.01em] transition-[background-color] duration-300 ease-[cubic-bezier(0.4,0,0.6,1)] focus-visible:outline-none focus-visible:[box-shadow:0_0_0_2px_var(--color-accent-solid)]";
