...
- The preview middleware's `/.sim/exec` endpoint requires a bearer token. The token is printed when `headless-serve-sim` starts and is stored in the per-device state file under `$TMPDIR/headless-serve-sim/server-{udid}.json`. Use `Authorization: Bearer <token>`.
...
```sh
ls $TMPDIR/headless-serve-sim/server-*.json
